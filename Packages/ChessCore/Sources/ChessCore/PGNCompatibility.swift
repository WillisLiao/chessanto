...
enum PGNCompatibility {
    ...
    public static func parseSAN(_ san: String, in position: Position) -> Move? {
        ...
        // For all other tokens, delegate to upstream SANParser; retry without
        // check/mate suffixes because upstream's patterns reject them on some
        // shapes (castling in particular: "O-O-O+").
        return SANParser.parse(move: san, in: position) ?? (san != clean ? SANParser.parse(move: clean, in: position) : nil)
    }
