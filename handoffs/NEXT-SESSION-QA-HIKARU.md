# Session: QA hardening - Hikaru games
...
```
git worktree add -b qa/hikaru-games ../chessanto-qa-hikaru main
```

Do all your work inside `../chessanto-qa-hikaru`.
...
Fetch every public archive for chess.com user `Hikaru` via `ChessComKit`
and, for every single game, run it end-to-end through PGN parse, full-game
`ChessGame` replay of every ply, and report building
...
- Don't touch other players' games (see the sibling QA sessions for
  Carlsen, Caruana, and edge cases).
...
Write `devlogs/<date>-qa-hikaru.md` describing what you found, what you
fixed, and the exact verification output.
Add a new `## Current state - QA Hikaru games` section at the TOP of
`handoffs/HANDOFF.md` (do not delete existing history there).
Commit everything on your branch (`qa/hikaru-games`).
