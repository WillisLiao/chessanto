# Session: QA hardening - Carlsen games
...
```
git worktree add -b qa/carlsen-games ../chessanto-qa-carlsen main
```

Do all your work inside `../chessanto-qa-carlsen`.
...
Fetch every public archive for chess.com user `MagnusCarlsen` via
...
Write `devlogs/<date>-qa-carlsen.md` describing what you found, what you
fixed, and the exact verification output.
Add a new `## Current state - QA Carlsen games` section at the TOP of
`handoffs/HANDOFF.md` (do not delete existing history there).
Commit everything on your branch (`qa/carlsen-games`).
