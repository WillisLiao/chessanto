...
The prior instance's `ChessComGame.pgn` optionality fix was verified against that real data as a genuine shared-layer bug fix (strict decode threw on any archive containing a pgn-less game), with its compile-forced 7-line adaptation in `ChessComFetchView`.
